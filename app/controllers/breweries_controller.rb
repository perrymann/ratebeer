class BreweriesController < ApplicationController
  ...
  def index
    ...
    @retired_breweries = case @order
      when 'name' then @retired_breweries = @retired_breweries.sort_by{ |b| b.name}
      when 'year' then @retired_breweries = @retired_breweries.sort_by{ |b| b.year}
    end
=begin
      if session[:reverse]
...
=end
  ...
  # POST /breweries.json
  def create
    ["brewerylist-name", "breweryrlist-year"].each{ |f| expire_fragment(f) }
